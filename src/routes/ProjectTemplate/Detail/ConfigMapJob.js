...
import EnvDataModal from './EnvDataModal';

class ConfigMapJob extends PureComponent {
  state = {
  ...
  onAdd = () => {
	...
	onDelete = (id,config_map_id) => {
	  console.log(id,config_map_id,"3333333");
	    const {dispatch,match} = this.props;
	    dispatch({
	      type: "cronjob/deleteConfigMapData",
	      payload: {
	        id: id,
  ...
  render() {
    ...
    return (
      <div>
        ...
        >
          {configMapInfo && configMapInfo.id ? (
            <div>
              ...
              <p>创建时间: {configMapInfo.CreatedAt ? configMapInfo.CreatedAt : ''}</p>
            </div>
          ) : ('')}
        ...
        <EnvDataModal visible={this.state.envDataVisible} title={this.state.envDataTitle}
...
export default connect(({ cronjob }) => ({
  configMapInfo: cronjob.configMapInfo,
  configMapDataList: cronjob.configMapDataList,
  configMapDataPage: cronjob.configMapDataPage,
}))(ConfigMapJob);
